$(function () {
    ...
    function initMapObject(placeholderID, objectListID) {
        ...
        mapObject['latLng'] = L.latLng(0, 0);
        mapObject['lpMarker'] = null;
    ...
    function mapChecked(mapObject) {
        ...
        setTimeout(updateBounds(mapObject), 5000);        
    }
        ...
        $.getJSON(getVisualizerDataURL, function (data, status) {
            if (status === 'success' && data !== null) {
                ...
                if ('error' in data) {
                    ...
                    mapObject['mapBounds']['LatestPosition'] = bounds;
                        
                    // Add marker at the last coordinate
                    mapObject['lpMarker'] = L.marker(latestPosition);
                    mapObject['lpMarker'].addTo(mapObject['map']);
    ...
    function removeLatestPositionFromMap(mapObject) {
        mapObject['map'].removeLayer(mapObject['lpMarker']);
            
        //remove the bounds and re-center/re-zoom the map
        delete mapObject['mapBounds']['LatestPosition'];
        
        updateBounds(mapObject);
    }

    function addGeoJSONToMap(mapObject, dataObjectJsonName) {
        var getVisualizerDataURL = siteRoot + 'api/getDashboardObjectVisualizerDataByJsonName/' + cruiseID + '/' + dataObjectJsonName;
        $.getJSON(getVisualizerDataURL, function (data, status) {
            if (status === 'success' && data !== null) {
    ...
    function addTMSToMap(mapObject, tmsObjectJsonName) {
        var getDataObjectFileURL = siteRoot + 'api/getDashboardObjectVisualizerDataByJsonName/' + cruiseID + '/' + tmsObjectJsonName;
        $.getJSON(getDataObjectFileURL, function (data, status) {
            if (status === 'success' && data !== null) {
                ...
                if ('error' in data){
                    ...
                    // Build the layer
                    mapObject['tileLayers'][tmsObjectJsonName] = L.tileLayer(location.protocol + '//' + location.host + cruiseDataDir + '/' + data[0]['tileDirectory'] + '/{z}/{x}/{y}.png', {
                        tms:true,
                        bounds:L.latLngBounds(southwest, northeast),
                    });
                        
                    // Add the layer to the map
                    mapObject['tileLayers'][tmsObjectJsonName].addTo(mapObject['map']);
    ...
    function removeTMSFromMap(mapObject, tmsObjectJsonName) {
        //remove the layer
        mapObject['map'].removeLayer(mapObject['tileLayers'][tmsObjectJsonName]);
        delete mapObject['tileLayers'][tmsObjectJsonName];
    ...
    $.each(mapObjects, function(i) {
        $( '#' + mapObjects[i]['objectListID']).find(':checkbox:checked').change(function() {
            if ($(this).is(":checked")) {
                ...
                if ($(this).hasClass("lp-checkbox")) {
                    //alert($(this).val());
                    removeLatestPositionFromMap(mapObjects[i]);
                } else if ($(this).hasClass("geoJSON-checkbox")) {
                    //alert($(this).val());
